...
def parse_config(config_file: dict, interactive: bool = False) -> dict or str:
    ...
    VALIDITY: dict = {"drive": {"func": common.get_drives, 
                                "mode": "execute", 
                                "default": ""}, 
                      "root_url": {"func": common.check_url, 
                                   "mode": "check", 
                                   "default": "https://repo.xenialinux.com/releases/current/root.img",
                                   "valid_text": "a URL that points to a root.img"}, 
                      "filesystem": {"func": common.get_fs,
                                     "mode": "execute",
                                     "default": "btrfs"}}
    ...
    for key in VALIDITY:
        ...
        match VALIDITY[key]["mode"]:
            ...
            case "check":
                if key not in config_file or not VALIDITY[key]["func"](value=config_file[key]):
                    config_file = fix_config(config_file, interactive, key, VALIDITY[key]["valid_text"], default)
